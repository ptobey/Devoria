...
while kill -0 "$server_pid" 2>/dev/null; do
    ...
    if grep -Fq "[Devoria] Enabling Devoria v" "$server_directory/server.log" \
        ...
        while ! grep -Fq "ModelEngine is unavailable; model-backed mobs cannot be summoned." "$server_directory/server.log"; do
            if ! kill -0 "$server_pid" 2>/dev/null || (( SECONDS >= command_deadline )); then
                echo "The unavailable ModelEngine command did not return its fallback message:" >&2
                tail -n 200 "$server_directory/server.log" >&2
                exit 1
            fi
            sleep 1
